...
- **Kết quả các lượt chạy** (runner `eval/run_eval.py`, gọi AI thật, mọi lượt lưu đủ từng case kể cả fail):

  | Lượt | Model | Bộ | Đo được | Tổng thể | classify (bar ≥80%) | Lớp ① (bar 100%) | Kết luận |
  |---|---|---|---|---|---|---|---|
  | **001** — lượt đo đầu | gemini-2.5-flash | 35 | 35/35 | **29/35 = 82,9%** | 11/13 = 84,6% ✅ | **7/8** ❌ | **Chưa đạt bar** — điều kiện cứng thất bại |
  | 002 | gemini-2.5-flash | 35 | 23/35 | không dùng | — | 8/8 ✅ | Hết quota giữa đường. Fix ① có tác dụng nhưng lộ regression lớp ② |
  | 003 | gemini-2.5-flash | 35 | 6/35 | không dùng | — | — | Hết quota ngày, không hợp lệ |
  | 004 | gemini-2.5-flash | 38 | 9/38 | không dùng | — | — | Hết quota. Lượt đầu sau merge — xác nhận taxonomy mới làm lệch nhãn |
  | 005 | openai:gpt-4o-mini | 38 | 38/38 | 35/38 = 92,1% | 16/16 ✅ | **7/9** ❌ | Đầy đủ trên code hiện tại nhưng điều kiện cứng vẫn thất bại |
  | 006 | openai:gpt-4o-mini | 38 | 38/38 | 37/38 = 97,4% | 16/16 ✅ | 9/9 ✅ | Sửa 2 bug tách token → lớp ① đạt. Còn 1 lỗi crash schema |
  | **008** — chốt | **openai:gpt-4o-mini** | 38 | 38/38 | **38/38 = 100%** | **16/16 = 100%** ✅ | **9/9** ✅ | **ĐẠT quality bar** cả hai điều kiện |

  Lượt 007 giống 008 về kết quả (38/38) nhưng dòng `Model` bị ghi sai — xem mục ghi chú trong chính file đó. Chi tiết từng case: `eval/run_00N.md`.

  ⚠️ **Cách đọc bảng này cho trung thực**: 100% ở lượt 008 **không có nghĩa sản phẩm không còn lỗi**. Nó có nghĩa là bộ 38 case hiện tại đã được sản phẩm xử lý đúng hết — và mỗi lượt đo đều tìm ra bug thật mà lượt trước không thấy. Bộ test cần được làm khó thêm (mục việc còn phải làm ở §8b) thay vì coi 100% là đích.
...
- **Phân tích khoảng cách với bar** (nội dung slide 4 khi demo):
  ...
  4. **Hai bug tách token khiến agent nói sai "chưa thấy"** (tìm ra ở lượt 005, sửa ở 006): (a) danh sách stopword tiếng Việt **không được bỏ dấu** bằng cùng hàm với token, nên gần như không stopword nào bị loại — `bằng` trong danh sách không bao giờ khớp token đã bỏ dấu `bang`; (b) **dấu câu dính vào token** — `lệnh:` thành `lenh:` nên không khớp `lenh`. Hai lỗi cộng lại làm điểm khớp của case `C4_04` tụt từ 0,67 xuống 0,33 → dưới ngưỡng → agent trả lời "Chưa thấy nội dung đó được nhắc" rồi *ngay sau đó vẫn trích đúng câu lệnh* — câu trả lời tự mâu thuẫn, học viên không biết tin phần nào.
  5. **Prompt và schema xung đột làm sập lượt hỏi** (tìm ra ở lượt 006, sửa ở 008): `ROUTE_INTENT_PROMPT` nói rõ `window_minutes` có thể là `null`, nhưng `IntentRoute` khai `float` cứng → khi model trả `null` đúng như prompt cho phép, pydantic raise `ValidationError` và **toàn bộ câu hỏi của học viên bị lỗi** (case `Q03`). Đã sửa schema nhận `None` rồi quy về mặc định 10 phút, **và** thêm guardrail ở `route_intent_node`: mọi structured output không hợp lệ đều rơi về `unclear` để hỏi lại thay vì làm sập — đường an toàn nhất vì không đoán, không bịa.
  6. **Hai case là lỗi THIẾT KẾ TEST, không phải lỗi sản phẩm** (lượt 005): `C1_04` sản phẩm hỏi lại "bạn muốn giải thích biểu đồ nào?" và `C1_05` sản phẩm từ chối vì ngoài phạm vi — **cả hai đều không bịa**, tức đạt đúng điều nhóm quan tâm, nhưng bị chấm fail vì test cũ chỉ chấp nhận đúng một cách diễn đạt "không tìm thấy". Đã đổi sang tiêu chí `must_not_fabricate` (đạt nếu thừa nhận thiếu căn cứ **hoặc** hỏi lại **hoặc** từ chối; vẫn fail nếu mô tả nội dung không tồn tại). Ghi rõ để người chấm tự đánh giá xem việc nới này có chính đáng.
  7. **Việc còn phải làm**: bộ test đã bị sản phẩm "học hết" (38/38) nên cần làm khó thêm — ưu tiên case ④ về code/công thức nhiều biến thể, case ② tin nhắn nửa tiếng Anh nửa tiếng Việt, và case ① khi transcript có nội dung *gần giống nhưng không phải* thứ học viên hỏi (bẫy khó nhất, hiện chưa có case nào).
...
| Sau khi merge nhánh teammate | `llm.py` viết lại để nhận **cả OpenAI và Gemini**, tự chọn theo key có trong `.env`, giữ cơ chế xoay vòng nhiều key; chặn trường hợp tên model không thuộc họ của provider | Nhánh merge về đổi sang OpenAI-only làm 5 key Gemini của nhóm hết dùng được; và `.env` để `VLEARNNOTE_MODEL=gemini-2.5-flash` trong khi chạy key OpenAI là cái bẫy sinh lỗi rất khó hiểu |
| Sau khi merge nhánh teammate | Golden set 35 → **38 case**: thêm `C4_05`/`C1_09` kiểm nhãn `student_insight`, `S07` kiểm nhận diện `speaker`; thêm tiêu chí `label_in` | Teammate mở rộng taxonomy lên 9 nhãn — trong đó quy tắc "phát biểu của học viên không được ghi thành kiến thức chuẩn" là ràng buộc domain quan trọng cần có case kiểm; đồng thời `definition`/`key_point`/`insight` chồng lấn nhau nên đo bằng một nhãn cứng là thước đo yếu |
| Sau lượt đo 005 | Sửa 2 bug trong `_content_tokens`: bỏ dấu cho stopword bằng cùng hàm với token, và cắt dấu câu ở hai đầu token | Agent trả lời "chưa thấy nội dung đó được nhắc" rồi vẫn trích đúng câu lệnh ngay sau — câu trả lời tự mâu thuẫn (case `C4_04`), làm thất bại điều kiện cứng lớp ① |
